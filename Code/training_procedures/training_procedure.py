...
class TrainingProcedure(ABC):
    @abstractmethod
    def train_base(self, net, trainloader, verbose, load_path, save_path, n_epochs, losses_breaks):
        """
        abstract method to train the base model (for task 0). Params:
        net: model to be traineed
        trainloader: loader with training data
        verbose: if set to True, print additional info
    ...
    @abstractmethod
    def train_CL(self, net, prevModel, trainloader, task, verbose, n_epochs, losses_breaks):
        """
