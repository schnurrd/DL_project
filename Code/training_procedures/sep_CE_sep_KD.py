        ...
        for epoch in range(n_epochs):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(trainloader, 0):
                ...
                for it in range(task):
                    loss += task*klDivLoss(F.log_softmax(oldClassOutputs[:, it*globals.CLASSES_PER_ITER:(it+1)*globals.CLASSES_PER_ITER], dim=-1), F.softmax(prevOutputs[:, it*globals.CLASSES_PER_ITER:(it+1)*globals.CLASSES_PER_ITER], dim=-1))
